...
#pragma HLS INTERFACE s_axilite port=imageWidth bundle=CTRL
   ...
   // Max pooling layer4
   // int pool_size = 2;
    double pool4Output[7*7*128];
    maxPooling(conv4Output, pool4Output, imageWidth4 - kernelWidth4 + 1 ,num_kernels4, pool_size);


    // fully connected layer. 
    char a_f2='R';
    double fully1output[512];
    fullyConnectedLayer(pool4Output,fully1output,dense1_weights,dense1_bias,6272,512,a_f2);
    
    char a_f3='S';
    double fully2output[3];
    fullyConnectedLayer(fully1output,fully2output,dense2_weights,dense2_bias,512,3,a_f3);
    char a_f4='S';
    if(a_f4=='S'){
        softmax(fully2output,3,output);
    }
}
